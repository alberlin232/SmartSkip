...
class Builder {
 public:
  ...
  RadixSpline<KeyType>* Finalize() {
    ...
    return new RadixSpline<KeyType>(
 ...
 private:
  ...
  // // KeyType == uint8_t.
  // static size_t GetNumShiftBits(uint8_t diff, size_t num_radix_bits) {
  //   const uint8_t clzl = __builtin_clzl(diff);
  //   if ((8 - clzl) < num_radix_bits) return 0;
  //   return 8 - num_radix_bits - clzl;
  // }
